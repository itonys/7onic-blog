...
| 균일한 문장 퀄리티 | 모든 문단이 비슷한 수준으로 잘 다듬어져 있음 | 진짜 사람 글은 어떤 문단은 명문이고 어떤 문단은 대충 씀. 불균일함이 사람 신호 |
| 계산적 자기 비하 | 매 섹션 적절한 타이밍에 적절한 양의 "embarrassingly" | 한 곳에서 과하게 자조하고 나머지는 담담하게. 고르게 분산된 겸손 = AI 시그널 |
| 균일한 섹션 구조 | 모든 섹션이 "상황→코드→경험→해결" 동일 아크 | 어떤 섹션은 코드 없이 3줄, 어떤 섹션은 길게. 예측 가능한 구조 = 기계적 |
| 도입부 균형 잡힌 나열 | "new syntax here, renamed API there, a few snippets" | 3박자 균형 나열은 편집자가 다듬은 느낌. 하나만 콕 집거나 불균형하게 |
| 깔끔한 최종 요약 | "The model is better. Debugging is clearer. color-mix killed a burden." | 모든 포인트를 회수하는 결론은 에세이 과제. 불완전/열린 마무리가 자연스러움 |
...
- **섹션 길이·구조 변주** — 긴 섹션 다음에 3줄짜리 섹션. 코드 없는 섹션. 모든 섹션이 같은 템플릿이면 기계적
- **자조는 한 곳에 몰기** — 한 섹션에서 실컷 자기를 까고, 나머지에서는 담담하게. 분산하면 계산적으로 보임
- **날것 문단 허용** — 모든 문단을 같은 수준으로 다듬지 않는다. 대충 쓴 "whatever, it works" 문단이 섞여야 진짜 사람
...
**모든 포스트** 맨 끝(About footer 바로 위)에 다음 포스트 티저를 넣는다. 시리즈 여부와 관계없이 필수.

```md
---

*Next: 다음 포스트 내용을 한 줄로 티저. 제목이 아닌, 독자가 궁금해할 만한 hook.*
