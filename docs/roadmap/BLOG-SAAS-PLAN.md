...
| 기능 | Free | Basic (500엔/월) | Pro (1000엔/월) | Enterprise |
|---|---|---|---|---|
| 블로그 수 | 1개 | 1개 | 3개 | 커스텀 |
| 도메인 | `username.blog.7onic.app` | 커스텀 도메인 | 커스텀 도메인 | 커스텀 도메인 |
| **월 포스트** | **3개** | **20개** | **100개** | **커스텀** |
| **자동 교차게시 플랫폼** | dev.to + Qiita (2개) | + Hashnode (3개) | + Zenn + はてなブログ (5개) | 커스텀 |
| 수동 복사 (Velog/Tistory/Medium) | ✅ | ✅ | ✅ | ✅ |
| 테마 선택 | 1종 고정 | 전체 | 전체 | 전체 + 커스텀 |
| 브랜드 컬러 | ✅ | ✅ | ✅ | ✅ |
| 로고/파비콘 | ✅ | ✅ | ✅ | ✅ |
| 메뉴/카테고리/시리즈 | ✅ | ✅ | ✅ | ✅ |
| 작성자 정보 | ✅ | ✅ | ✅ | ✅ |
| 소셜 링크 | ✅ | ✅ | ✅ | ✅ |
| OG 이미지 | 기본 템플릿 | 기본 템플릿 | 커스텀 가능 | 완전 커스텀 |
| Google Analytics | ❌ | ✅ | ✅ | ✅ |
| 댓글 (Giscus) | ❌ | ✅ | ✅ | ✅ |
| 미디어 스토리지 | 50MB | 500MB | 2GB | 커스텀 |
| SLA | 없음 | 없음 | Best effort | 99.9% 보장 |
| 전담 지원 | ❌ | ❌ | 이메일 | 슬랙 채널 + 우선순위 |

> ⚠️ **"무제한" 표기 금지** — 모든 플랜에 명시적 상한. Pro 100개/월을 초과하는 유저는 Enterprise 개별 계약.

### Rate Limit (모든 플랜 공통, 스팸/악용 방지)

플랜 수량과 별개로 단기간 대량 게시 차단:
...
| 일 최대 게시 | 30개 (Pro 기준) |
| 동일 IP 계정 생성 | 5개까지 |
| 신규 가입 7일간 자동 게시 | 일 5개로 제한 (워밍업) |

### 월 한도 초과 시 UX

- ❌ 게시 차단 (하드 리밋)
- ✅ draft로 저장 (삭제 X) + 업그레이드 CTA 표시
- ✅ 다음 달 1일 자동 리셋
- ✅ 남은 할당량 대시보드에 실시간 표시 (e.g., "3/3 사용")
...
| 9 | **교차배포 알림 로직** | 현재 CLI 스크립트는 터미널에서 에러 즉시 확인 가능. SaaS는 서버가 유저 대신 배포하므로 유저가 결과를 알 수 없음 → 알림 시스템 필수. 이벤트: rate limit(429) / 인증 만료 / 플랫폼 장애(5xx) / 부분 성공. 채널: 인앱 알림(필수) / 이메일(Free 이상) / 웹훅(Pro 이상) |
...
| 6 | **교차배포 알림 시스템** | 유저가 대시보드를 안 보고 있을 때 배포 결과를 어떻게 전달할 것인가. 채널(인앱/이메일/웹훅) × 이벤트(성공/실패/부분 성공/rate limit/인증 만료) 조합 설계 필요. 인앱은 필수, 이메일은 어느 플랜부터, 웹훅은 Pro 이상? |
