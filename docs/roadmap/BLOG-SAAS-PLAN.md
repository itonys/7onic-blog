...
### 🎯 현실 요약 — 연수입 100만엔 미만 인디 SaaS

> **앞 섹션들은 "규모 커졌을 때 대비" 청사진이다.** 지금 당장 실제로 구현해야 할 것만 골라낸 현실 버전.

#### 실제 필요한 것은 5개뿐

1. ✅ **Stripe 위임** — 결제 코드는 Stripe Checkout으로. 카드 정보 우리 서버 안 거침
2. ✅ **특상법 표시 페이지** — 푸터 링크 → `/legal/commerce-law`
3. ✅ **이용약관 1페이지** — 템플릿 복붙 (무료 템플릿 많음)
4. ✅ **개인정보처리방침 1페이지** — 템플릿 복붙
5. ✅ **유저 계정 삭제 버튼** — 분쟁 시 대응용

이게 전부. 변호사 리뷰 불필요 (연수입 100만엔이면 리뷰비가 더 나감).

#### 특상법 사업자 표시 — 개인사업자 (個人事業主) 확정

사업자 상태: **個人事業主** (開業届 제출, 法人 아님)

```
販売業者: 정현준 (Jung Hyunjun) ← 본명 표시 필수 (필명/Handle 불가)
屋号: 7onic
メール: contact@blog.7onic.app
住所・電話: 請求があり次第、遅滞なく開示します
```

**핵심**:
- 본명은 숨길 수 없음 (법적 필수)
- 주소·전화는 "청구 시 제공" 특례 가능
- 屋号(7onic)는 추가 표시 OK (본명과 별도)

**표시 위치:** 푸터 링크 1개 (`/legal/commerce-law`)로 끝. 클릭 1회로 도달하면 합법.

> 향후 법인 전환 시 이 섹션을 "법인명 + 대표자명" 방식으로 업데이트.
...
|-------|----------|
| ~100만엔 | 위 5개만 |
| 100~500만엔 | 약관 변호사 리뷰 1회 (~10만엔) |
| 500~1000만엔 | 세무사 계약, 소비세 과세사업자 전환 검토 |
| 1000만엔+ | 법인 전환 검토, 본격 컴플라이언스 |
| 1억엔+ | DPO 지정, SOC 2 검토 |
...
| 항목 | 무시 가능한 이유 |
|------|--------------|
| EU GDPR 완벽 준수 | 규제당국 레이더 아래, 인디 타겟 안 함 |
| SOC 2 감사 | 엔터프라이즈 B2B 진입 전까지 불필요 |
| 분기별 복구 훈련 | 백업만 있으면 충분 |
| 전담 보안 담당자 | 혼자 분기 1시간 체크로 충분 |
| 외부 침투 테스트 | 연수입 1000만엔+ 이후 고려 |
| 다중 클라우드 백업 | Phase 2 (유저 수백명 이후) |
| 이상 활동 탐지 시스템 | 수동 체크로 충분 |

#### 즉, 지금 이 Plan의 80%는 "미래 대비 청사진"

당장 구현: 위 5개 ✅
1~2년 안에 성장하면: Phase 1.5 🟡 추가
실제 중견 규모 도달: Phase 2 🟢 부터 본격 적용

**앞에 길게 쓴 보안/법령 섹션은 참고용 체크리스트.** 100만엔 규모에서는 5개만 지키면 99% 리스크 커버됨.
