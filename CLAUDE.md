...
**공통 원칙·승인 플로우·Red Flags**: `~/Documents/ai-project/docs/rules/shared/DOC-PROPAGATION-RULES.md` (모든 프로젝트 공통 SSOT).

**요약**:
- **작업 중**: AI는 요청한 것만 수정. 문서·POST-INDEX 등 전파 대상 건드리지 않음.
- **유저가 "커밋 푸시" 등 커밋 명령 시**: AI가 `git diff` 스캔 → 아래 매트릭스 대조 → 전파 계획 보고 + 승인 대기.
